...
import { createSpinResult, addPendingGift } from "../datas/spinResult";
  ...
  return (
    ...
    >
      ...
      <div className={`main-layout ${isMobile ? "main-layout-mobile" : "main-layout-desktop"}`}>
        <div
          className={`wheel-wrapper ${isPremiumMode ? "wheel-wrapper-premium" : ""}`}
        ...
        <div className={`form-wrapper ${isMobile ? "form-wrapper-mobile" : ""}`}>
          <h3 className={`form-title ${isPremiumMode ? "form-title-premium" : ""}`}>
            {isPremiumMode ? "✨ Vòng Quay VIP Sẵn Sàng ✨" : "🔒 Nhập Thông Tin Để Mở Vòng Quay VIP"}
          </h3>

          <div className="input-group">
            <input type="text" placeholder="Nhập tên bé..." value={babyName} onChange={(e) => setBabyName(e.target.value)} className="input-field" />
          </div>

          <div className="input-group">
            <input type="date" value={babyBirthday} onChange={(e) => setBabyBirthday(e.target.value)} required className="input-field input-date" />
          </div>
        </div>
      </div>

      <ListGiftPopup isOpen={showListPopup} onClose={() => setShowListPopup(false)} basicPrizes={basicRound} vipPrizes={vipRound} />
